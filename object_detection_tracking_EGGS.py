...
def video_detect(video_name):
    ...
    start_line_A = (0, 480)
    end_line_A = (1300, 480)
    # start_line_B = (1205, 480)
    # end_line_B = (1220, 480)
    # start_line_C = (1225, 480)
    # end_line_C = (1280, 480)
    color_damaged = (0, 0, 255)
    color_norm = (0, 255, 0)

    # Initialize the video capture and the video writer objects
    video_cap = cv2.VideoCapture(video_name)
    output_video_path = f"outputvideos/output.mp4"
    ...
    classes_path = "config/coco_eggs.names"
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            if class_id == 0:
                cv2.rectangle(frame, (x1, y1), (x2, y2), color_damaged, 3)
                cv2.rectangle(frame, (x1 - 1, y1 - 20),
                              (x1 + len(text) * 12, y1), color_damaged, -1)
            else:
                cv2.rectangle(frame, (x1, y1), (x2, y2), color_norm, 3)
                cv2.rectangle(frame, (x1 - 1, y1 - 20),
                              (x1 + len(text) * 12, y1), color_norm, -1)
            ...
            #print( "points[track_id] = ", points[track_id])
            ...
            if center_y > start_line_A[1] and start_line_A[0] < center_x < end_line_A[0] and last_point_y < start_line_A[1]:
                ...
                if class_id == 0:
                    ...
                    counter_Eggs += 1
                #print("cross the line -- text= ", text, ",", "class_id=" , class_id )
        ...
        cv2.putText(frame, "Cross line objects:", (10, 483), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 0, 0), 2) #text of line A
        #cv2.putText(frame, "B", (530, 483), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        #cv2.putText(frame, "C", (910, 483), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(frame, f"{counter_A}", (640, 483), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 0, 0), 2) # counter objects
        #cv2.putText(frame, f"{counter_B}", (620, 483), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        #cv2.putText(frame, f"{counter_C}", (1040, 483), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.putText(frame, "Eggs:", (10, 516), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 25), 2)  # text of counter eggs
        cv2.putText(frame, f"{counter_Eggs}", (640, 516), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 25), 2) # counter eggs
        cv2.putText(frame, "Damaged eggs:", (10, 546), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 2)  # text of counter damaged eggs
        cv2.putText(frame, f"{counter_Damaged}", (640, 546), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 2)  # counter damaged eggs
    ...
    textOut= f"Count eggs= {counter_Eggs}, count damaged eggs= {counter_Damaged}"
    return textOut, output_video_path